# CLAUDE.md - WearableGearReviews.com

## Project Identity

**Site:** https://wearablegearreviews.com/
**Codename:** WearableReviewPro
**Niche:** Wearable Tech & Health Device Reviews
**Mode:** Light-first (Clean Tech)
**Amazon Tag:** wearablegearreviews-20
...
Username: WearableReviewPro
App Password: qADR vj0m h9fa aGoH RBdT 4KvU
```
...
- **Voice:** The wearable tech expert testing everything on wrist, body, or ear
- **Tone:** Deep technical knowledge paired with real-world testing
- **NOT:** Spec-sheet only, hype-driven, brand-biased

### Color Palette
| Token | Hex | Usage |
|-------|-----|-------|
| Primary | #3B82F6 | Tech Blue - CTAs, highlights |
| Secondary | #10B981 | Health Green - wellness indicators |
...
### Systeme.io
```
Tag: wearablegearreviews-popup-wearableguide
Tag: wearablegearreviews-leadmagnet-smartwatchguide
```

### Amazon
```
Affiliate Tag: wearablegearreviews-20
```
...
**Title:** "Complete Smartwatch Comparison Guide 2025"
**Format:** Interactive PDF
**Contents:** Feature matrices, health tracking accuracy data, battery life comparisons

---

## Content Categories
1. Smartwatches - Apple, Samsung, Garmin, etc.
2. Fitness Trackers - Bands and basic trackers
3. Health Monitors - Blood pressure, glucose, etc.
4. Audio Wearables - Earbuds, hearing aids
5. Smart Rings - Oura, etc.

---

## Review Template Elements
- Health Tracking Accuracy (1-10)
- Battery Life Score (1-10)
- App Quality (1-10)
- Build Quality (1-10)
...
- ❌ Reviews without real wearing time
- ❌ Ignoring health tracking accuracy
- ❌ Only covering flagship devices
- ❌ Spec comparisons without real use
- ❌ Generic tech stock imagery

---

*Generated from Creative Command Center DNA: wearable-gear-hub.json*
...
1. **Load Memory Context**
   ```
   Search memory for: wearablegearreviews context state decisions
   ```
...
4. **Check Open Tasks**
   ```
   mcp_memory_get key="open_tasks" context="wearablegearreviews"
   ```
...
```markdown
## SESSION CONTEXT: Wearable Gear Reviews
Site: https://wearablegearreviews.com
Amazon Tag: wearablegearreviews-20

## RULES FOR THIS SESSION
1. ALWAYS use health-conscious wearable tech expert with medical awareness voice - NEVER deviate
2. Include affiliate tag wearablegearreviews-20 on all product links
...
Ready to work on Wearable Gear Reviews. What's the task?
```
...
mem0_add({
  ...
  text: "Decided to use glass morphism for cards on Wearable Gear Reviews",
  metadata: { site: "wearablegearreviews", type: "decision" }
})
...
```
wearablegearreviews_state          - Current project state
wearablegearreviews_decisions      - Key decisions made
wearablegearreviews_tasks          - Task tracking
wearablegearreviews_preferences    - Discovered preferences
wearablegearreviews_solutions      - Technical solutions
wearablegearreviews_session_{date} - Session notes
```
...
```javascript
mcp_memory_set({
  key: "wearablegearreviews_session_" + new Date().toISOString().split('T')[0],
  value: JSON.stringify({
    completed: [...],
    pending: [...],
    decisions: [...],
...
<?php
/**
 * Plugin Name: MCP Tools - Wearable Gear Reviews
 * Description: Custom MCP tools for https://wearablegearreviews.com
 * Version: 1.0.0
...
class MCP_Tools_WearableGearReviews {
    ...
    public function register_tools( $tools ) {
        ...
        $tools[] = [
            ...
            'category'    => 'Wearable Gear Reviews',
            'inputSchema' => [
                'type'       => 'object',
                'properties' => [
...
add_action( 'plugins_loaded', function() {
    MCP_Tools_WearableGearReviews::get_instance();
}, 20 );
```
...
- - `wgr_wearable_review` - Generate wearable device reviews
- `wgr_health_feature_analysis` - Analyze health tracking features
- `wgr_accuracy_comparison` - Compare sensor accuracy
...
### Site-Specific Rules: Wearable Gear Reviews
1. Voice: Health-focused, detail-oriented, trustworthy
2. Affiliate Tag: wearablegearreviews-20 on ALL Amazon links
3. Design: Teal health-focused, clean, wellness-oriented
4. Content: Health features focus, accuracy testing, medical awareness
5. Anti-patterns: Medical claims without evidence, spec-only reviews
...
| Resource | URL |
|----------|-----|
| WordPress Admin | https://wearablegearreviews.com/wp-admin |
| MCP Endpoint | https://wearablegearreviews.com/wp-json/mcp/v1/bearer/sse |
| Site Frontend | https://wearablegearreviews.com |
